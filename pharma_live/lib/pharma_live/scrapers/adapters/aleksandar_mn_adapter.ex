defmodule PharmaLive.Scrapers.Adapters.AleksandarMnAdapter do
  ...
  defp scrape_page(url, seen) do
    with {:ok, doc} <- Http.fetch_html(url, receive_timeout: 30_000) do
      products =
        doc
        |> Floki.find(".item.product.product-item")
  ...
  defp extract_product(node) do
    ...
    if out_of_stock do
      ...
      title = Http.text(node, ".product-item-link h3")

      if title == "" do
        nil
      else
        price_raw = Http.text(node, ".special-price .price, .price-final_price .price")
        link = Http.attr(node, ".product.photo.product-item-photo", "href")
