defmodule PharmaLive.Scrapers.Adapters.FitLabAdapter do
  ...
  @base_url "https://fitlab.rs/product-category/suplementi"
  ...
  defp scrape_page(url, seen) do
    with {:ok, doc} <- Http.fetch_html(url, receive_timeout: 45_000) do
      products =
        doc
        |> Floki.find(".products li.product, .products li, .product")
  ...
  defp extract_product(node) do
    title = Http.text(node, "h2, .woocommerce-loop-product__title")

    if title == "" do
      nil
    else
      price_raw = Http.text(node, ".price ins .woocommerce-Price-amount, .price .woocommerce-Price-amount")
      link = Http.attr(node, ".woocommerce-LoopProduct-link, .woocommerce-loop-product__link", "href")
      image = Http.attr(node, ".woocommerce-LoopProduct-link > img, .woocommerce-loop-product__link > img", "src")
