defmodule PharmaLive.Scrapers.Adapters.FourFitnessAdapter do
  ...
  @base_url "https://4fitness.rs/proizvodi"
  ...
  defp scrape_page(url, seen) do
    with {:ok, doc} <- Http.fetch_html(url, receive_timeout: 30_000) do
      products =
        doc
        |> Floki.find("ul.products.columns-3 li.product")
  ...
  defp extract_product(node) do
    out_of_stock = (node |> Floki.attribute("class") |> List.first("")) |> String.contains?("outofstock")
    ...
    if out_of_stock do
      ...
      if title == "" do
        ...
        price_raw = Http.text(node, ".price ins .amount, .price > .amount")
        link = Http.attr(node, ".woocommerce-LoopProduct-link", "href")
        image = Http.attr(node, ".attachment-woocommerce_thumbnail", "src")
