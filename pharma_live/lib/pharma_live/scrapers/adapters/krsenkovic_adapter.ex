defmodule PharmaLive.Scrapers.Adapters.KrsenkovicAdapter do
  ...
  @base_url "https://apotekakrsenkovic.rs/all-products"
    ...
    products =
      ...
      |> Enum.reduce_while({[], MapSet.new(), 0}, fn page_num, {acc, seen, empty_count} ->
        ...
        case scrape_page(url, seen) do
          ...
          {:error, _} ->
            next_empty = empty_count + 1
            if next_empty >= 1, do: {:halt, {acc, seen, next_empty}}, else: {:cont, {acc, seen, next_empty}}
        end
      end)
      |> elem(0)
  ...
  defp extract_product(node) do
    ...
    if out_of_stock do
      ...
      title = Http.text(node, ".product-item-name .product-item-link")
      ...
      if title == "" do
        ...
        image = Http.attr(node, ".product-image-photo", "src") || Http.attr(node, ".product-image-photo", "data-src")
