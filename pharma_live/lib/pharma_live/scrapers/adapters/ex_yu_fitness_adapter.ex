defmodule PharmaLive.Scrapers.Adapters.ExYuFitnessAdapter do
  ...
  defp scrape_category(base_url, seen) do
    ...
    |> Enum.reduce_while({[], seen}, fn page_number, {acc, seen_acc} ->
      url = "#{base_url}/page-#{page_number}/?items_per_page=128"
  ...
  defp extract_product(node) do
    ...
    if out_of_stock do
      ...
      title = Http.text(node, ".ut2-gl__name")

      if title == "" do
        nil
      else
        price_raw = Http.text(node, ".ty-price")
        link = Http.attr(node, ".product_icon_lnk", "href")
        image = Http.attr(node, ".product_icon_lnk > img", "src")
