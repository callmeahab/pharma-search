defmodule PharmaLive.Scrapers.Adapters.LivadaAdapter do
  ...
  @base_url "https://apotekalivada.rs/akcije-i-promocije/page/"
  ...
  def scrape(_source) do
    products =
      ...
      |> Enum.reduce_while({[], MapSet.new()}, fn page_num, {acc, seen} ->
        url = "#{@base_url}#{page_num}/?count=36"
  ...
  defp scrape_page(url, seen) do
    with {:ok, doc} <- Http.fetch_html(url, receive_timeout: 35_000) do
      products =
        doc
        |> Floki.find(".product-type-exinfo")
  ...
  defp extract_product(node) do
    ...
    if out_of_stock do
      ...
      if title == "" do
        ...
        price_raw = Http.text(node, ".price .woocommerce-Price-amount:last-child")
        link = Http.attr(node, ".porto-tb-featured-image > a", "href")
        image = Http.attr(node, ".porto-tb-featured-image > a > img", "src") || Http.attr(node, ".porto-tb-featured-image > a > img", "data-src")
