defmodule PharmaLive.Scrapers.Adapters.MelisaAdapter do
  ...
  @base_url "https://apoteka.melisa.rs/shop"
  ...
  defp scrape_page(url, seen) do
    with {:ok, doc} <- Http.fetch_html(url, receive_timeout: 35_000) do
      products =
        doc
        |> Floki.find(".product-content")
        |> Enum.map(&extract_product/1)
        |> Enum.reject(&is_nil/1)

      has_next = doc |> Floki.find(".tb-icon-chevron-right") |> Enum.any?()
  ...
  defp extract_product(node) do
    ...
    if out_of_stock do
      ...
      if title == "" do
        ...
        price_raw =
          Http.text(node, ".price ins")
          |> case do
            "" -> Http.text(node, ".price")
        ...
        image =
          Http.attr(node, ".product-image > img", "data-src") ||
            Http.attr(node, ".product-image > img", "src") ||
            Http.attr(node, ".product-image > img", "data-original")
        ...
        %Product{
          ...
          currency: "RSD",
          in_stock: true,
          raw_payload: %{price_raw: price_raw, image: image, category: "Melisa"}
        }
      end
    end
  end
