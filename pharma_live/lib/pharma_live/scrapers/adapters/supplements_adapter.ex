defmodule PharmaLive.Scrapers.Adapters.SupplementsAdapter do
  ...
  @base_url "https://supplements.rs/shop"
  ...
  defp scrape_page(url, seen) do
    with {:ok, doc} <- Http.fetch_html(url, receive_timeout: 30_000) do
      products =
        doc
        |> Floki.find(".products.elementor-grid li")
  ...
  defp extract_product(node) do
    out_of_stock = node |> Floki.find(".stock.unavailable") |> Enum.any?()
