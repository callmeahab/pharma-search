    ...
    const products = await page.$$eval('ul > .product', (elements) => {
      return elements
        .map((element) => {
          const title =
            element
              .querySelector('h2.woocommerce-loop-product__title a')
              ?.textContent?.trim()
              .replace(',', '.') || '';
          ...
          let price = '';
          const priceElement = element.querySelector('.price_wrap .price');
          const salePriceElement = priceElement?.querySelector(
          ...
          const link = element.querySelector('.post_featured > a')?.getAttribute('href')
            || element.querySelector('h2.woocommerce-loop-product__title a')?.getAttribute('href')
            || '';
          const img = element.querySelector('img.attachment-woocommerce_thumbnail')?.getAttribute('src')
...
async function hasNextPage(page: Page): Promise<boolean> {
  try {
    const nextButton = await page.$('.next.page-numbers, a.next.page-numbers, .woocommerce-pagination a.next');
