...
const baseUrls = ['https://fillyfarm.rs/prodavnica/'];
    ...
    const products = await page.$$eval('.product', (elements) => {
      return elements
        .map((element) => {
          ...
          const link =
            element.querySelector('.thumbnail > a')?.getAttribute('href') || '';
          const img =
            element
...
async function hasNextPage(page: Page): Promise<boolean> {
  try {
    const nextButton = await page.$('.far.fa-angle-double-right');
...
scrapeMultipleBaseUrls().then(async (allProducts) => {
  if (allProducts.length > 0) {
    await insertData(allProducts, 'Filly');
